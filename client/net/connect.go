...
func (self *Net) Connect(
	window *window.Window,
	output output.Output,
	settings *settings.Settings,
) (_stream *quic.Stream, _closeConnection func(), _err error) {
	...
	if err != nil {
		...
		if ok {
			return self.Connect(window, output, settings)
		}

		return nil, func() {}, retErr
	}
	...
	if err != nil {
		closeConn(conn)
		return nil, func() {}, fmt.Errorf("Could not open stream:\n%v", err)
	}

	err = lib.StreamSendDatalenString(stream, settings.ServerPassword)
	if err != nil {
		lib.StreamSendEOFUnchecked(stream)
		closeConn(conn) // TODO: actually, maybe make a lib function like the one above AND maybe rename the function above
		return nil, func() {}, fmt.Errorf("Could not send password:\n%v", err)
	}
